...
/**
 * Robust cleanup: close the DB handle before removing files.
 * On Windows, better-sqlite3 holds WAL file locks that prevent immediate deletion.
 */
function safeCleanup(dir: string, db: ReturnType<typeof openDatabase>): void {
	...
	try {
		...
		// On Windows, files may still be briefly locked; retry after a short delay
		setTimeout(() => {
			try {
				rmSync(dir, { recursive: true, force: true });
			} catch {
...
describe('HostelGrievance API baseline', () => {
	...
	let dbRef: ReturnType<typeof openDatabase>;

	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), 'hg-api-'));
		dbRef = openDatabase(join(dir, 'hostel.db'));
		const uploadDir = join(dir, 'uploads');
		seedDatabase(dbRef, uploadDir);
		app = createApp({ db: dbRef, uploadsDir: uploadDir });
	});

	afterEach(() => {
		safeCleanup(dir, dbRef);
	});
